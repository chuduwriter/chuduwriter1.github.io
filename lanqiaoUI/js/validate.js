...
! function($) {
    ...
    Validator.prototype = {
        ...
        onAjaxSuccess: function(ele, rs) {
            ...
            // 服务器返回0或者true时代表验证通过
            // 执行验证成功或失败的业务处理
            if(rs == "true" || (rs.status || "") == "success") {

                this.removeErroredItem(ele);
                this.doAjaxSuccess(ele);

                // 当用户点击提交Ajax校验还未完成时
                // 最后一个校验完成且其余验证完成时后自动提交表单
                ...
                // 最后一个Ajax验证失败后删除正在提交标识
                if(this.waitingList.length == 0) {
                    this.removeSubmiting();
                }
        ...
        needNotCheck: function(ele, force) {
            ...
            var cachedForm = this.cachedDOM[ele.name],
                ...
                ignoreVal = $(ele).data("vd-ignore-val");

            // 页面初始值不进行校验
            // 如Ajax取得的数据不需要校验,在ele中添加data-vd-ignore-val
            if(isInitVal || (ignoreVal && ignoreVal === value)) {
                this.removeErroredItem(ele);
                return true;
            }
