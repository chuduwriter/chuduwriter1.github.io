...
! function($) {
    ...
    var Dialogor = function(options) {
        ...
        t.opts = $.extend(defaults = {
            ...
            _closeInterval: 1200 // 过多少秒后关闭对话框

        }, options);

        t.opts._refresh = !t.opts._success; // 提交成功时是否自动刷新页面(有成功回调函数时不刷新,提交失败不刷新)
    ...
    Dialogor.prototype = {
         ...
         * 绑定预处理事件
         */
        bindPrepareEvent: function() {

            var t = this;

            Global.GLB_Selector.init(t._dialog);
            Global.GLB_UploadFile.init(t._dialog);
        ...
        submitDialogForm: function(form, t) {
            ...
            $.post(url, data).done(function(rs) {
                ...
                // 显示成功提示信息
                t.showSubmitResultTip(rs);
                // 执行用户自定义操作
                t.opts._success && t.opts._success(rs);

                // 关闭对话框
                window.setTimeout(function() {
                    t.opts._autoClose && t.closeDialog();
                }, t.opts._closeInterval);
         ...
         * 在对话框内添加提示信息
         *
         * @param rs ajax提交成功返回值(未传入代表失败的情况)
         */
        showSubmitResultTip: function(rs) {
            ...
            if(rs) {
                ...
                if(rs.status == "success") {
                    msg = rs.message || "操作成功", style = "correct";
                } else {
                    msg = rs.message || "操作失败", style = "info";
                }
